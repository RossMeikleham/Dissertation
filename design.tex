...
and we can easily determine the execution path of code at compile time.
...
Currently preprocessor directives aren't supported in GPC so in the compiler we can
...
as the version compiled by GPC and run on the GPRM. 

This is especially useful for implementing the compiler as it allows for
...
programs already written in C++ to GPC.
...
Essentially this is implicitly cast to:
...
\item The condition part of the the for loop must result in a "pure" boolean type, a GPRM::Kernel boolean
     type is not allowed.
...
as well as completel compatibility with the C++ language.  
